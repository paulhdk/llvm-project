    ...
    if (const auto *VD = dyn_cast_or_null<VarDecl>(D)) {
      ...
      if (T.isTrivialType(VD->getASTContext())) {
        if (auto *BO = dyn_cast<BinaryOperator>(S)) {
          llvm::dbgs() << "\nMade it into BO if\n";
          auto *LHSExp = BO->getLHS()->IgnoreParenCasts();
          if (const auto *DRE = dyn_cast<DeclExpr>(LHSExp)) {
            const ValueDecl *VDec = DRE->getDecl();
            if (Ctx.lookup(VDec)) {
              llvm::dbgs() << "\nMade it into innermost if\n";
              Ctx = VMap->updateDefinition(VD, BO->getRHS(), Ctx);
            }
          }
        } else {
          llvm::dbgs() << "\nMade it to other if\n";
...
// blocks that strictly dominate the current block.  We do not bother to
// insert proper phi nodes, because they are not used in our analysis;
// instead, wherever a phi node would be required, we simply remove that
// definition from the context (E.g. x above).
//
...
// might have to insert a phi node.)  On the second pass, these definitions
// are set to NULL if the variable has changed on the back-edge (i.e. a phi
...
/// Find the appropriate source locations to use when producing diagnostics
...
public:
  ...
  void handleUnlock(FactSet &FSet, FactManager &FactMan,
    ...
    for (const auto &UnderlyingMutex : UnderlyingMutexes) {
      ...
      // on double unlocking/locking if we're not destroying the scoped
      // object.
...
void ThreadSafetyAnalyzer::getEdgeLockset(FactSet &Result,
  ...
  // If the condition is a call to a Trylock function, then grab the
  // attributes
...
/// else output error messages related to missing locks.
...
/// \param Self  If \p Exp = nullptr, the implicit this argument or the
/// argument
...
/// check whether we hold any required mutexes.
...
void BuildLockset::VisitReturnStmt(const ReturnStmt *S) {
  ...
  // If returning by reference or pointer, check that the function requires
...
bool ThreadSafetyAnalyzer::join(const FactEntry &A, const FactEntry &B,
                                bool CanModify) {
  if (A.kind() != B.kind()) {
    // For managed capabilities, the destructor should unlock in the right
    // mode anyway. For asserted capabilities no unlocking is needed.
...
/// lockset of each branch being merged. For example, given the following
/// sequence: A; if () then B; else C; D; we need to check that the lockset
/// after B and C are the same. In the event of a difference, we use the
/// intersection of these two locksets at the start of D.
///
...
/// held at the end of each block, and issue warnings for thread safety
/// violations. Each block in the CFG is traversed exactly once.
void ThreadSafetyAnalyzer::runAnalysis(AnalysisDeclContext &AC) {
  ...
  if (!SortedGraph->empty()) {
    ...
    for (const auto *Attr : D->attrs()) {
      ...
      if (const auto *A = dyn_cast<RequiresCapabilityAttr>(Attr)) {
        ...
        // implementation. We must ignore such methods.
        if (A->args_size() == 0)
          return;
        getMutexIDs(A->isShared() ? SharedLocksToAdd : ExclusiveLocksToAdd, A,
                    nullptr, D);
  ...
  for (const auto *CurrBlock : *SortedGraph) {
    ...
    // block (FirstLoopBlock) we need to check that the Lockset of Block is
    // equal to the one held at the beginning of FirstLoopBlock. We can look
    // up the Lockset held at the beginning of FirstLoopBlock in the
    // EntryLockSets map.
    for (CFGBlock::const_succ_iterator SI = CurrBlock->succ_begin(),
                                       SE = CurrBlock->succ_end();
         SI != SE; ++SI) {
